...
sz = 508;
sz2 = 508;
sz3 = 44; % modify 1, define sz, sz2, and sz3 according to the size of the 3D image

fiberstack = zeros(sz,sz2,sz3);

for i = 1:sz3
    rawima = imread(['D:\Order index calculation\Examples\',num2str(i),'.tif']); % modify 2，the path should be modified
    rawima = sum(rawima,3);
    rawima = double(rawima);
    fiberstack(:,:,i) = rawima;
    % Here rawima is the raw intensity image
...
for mm = 1:sz3
    ...
    imwrite(double(orderprettyima),['D:\3D order index maps\',num2str(mm),'.tif'],'tif'); % modify 3
end
% Here I create a folder named "3D order index maps" to save the order
% index maps one by one generated in a 3D format




%% Here to calculate the order index of fiber-like structures in a 2D context
fiberima = imread(['D:\Order index calculation\Examples\11.tif']); % modify 1, the path should be modified
fiberima = sum(fiberima,3);
% Define the path of the folder where the images are saved
...
imwrite(double(ordermappretty),['D:\2D order index map.tif'],'tif'); % modify 3
% Here I save the 2D order index map
